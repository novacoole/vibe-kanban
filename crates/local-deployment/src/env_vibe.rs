...
/// * `branch_name` - The branch name to use for `{{ branch() }}` placeholders
/// * `used_ports` - Set of ports already in use by other worktrees
///
/// # Returns
/// `EnvVibeResult` containing processed content and assigned ports
pub fn process_env_vibe_template(
    ...
    for line in content.lines() {
        ...
        while auto_port_re.is_match(&processed_line) {
            let port = find_available_port(used_ports, &file_ports)?;
...
/// Find an available port that's not used by other worktrees or the system.
...
mod tests {
    ...
    fn test_auto_port_replacement() {
        ...
        let result = process_env_vibe_template(content, "main", &used_ports).unwrap();
    ...
    fn test_auto_port_avoids_used_ports() {
        ...
        assert!(port >= 60000 && port <= MAX_PORT);
    }
    ...
    fn test_branch_replacement() {
        ...
        let result =
            process_env_vibe_template(content, "vk/feature-branch", &used_ports).unwrap();
    ...
    fn test_branch_with_default_uses_branch() {
        ...
        let result =
            process_env_vibe_template(content, "vk/my-branch", &used_ports).unwrap();
    ...
    fn test_branch_with_default_uses_default() {
        ...
        let result = process_env_vibe_template(content, "", &used_ports).unwrap();
    ...
    fn test_mixed_placeholders() {
        ...
        let result =
            process_env_vibe_template(content, "feature/login", &used_ports).unwrap();
    ...
    fn test_whitespace_variants() {
        ...
        for content in test_cases {
            let result = process_env_vibe_template(content, "main", &HashSet::new()).unwrap();
    ...
    fn test_port_availability_check() {
        // This test verifies the socket binding check works
        // We can't easily test an unavailable port, but we can verify
        // the function returns true for a likely-available high port
        ...
        assert!(available || !available); // tautology to show it runs
    }
...
DEBUG=true"#;
        ...
        let result =
            process_env_vibe_template(content, "feature/auth", &HashSet::new()).unwrap();
        ...
        assert!(result.processed_content.contains("DB_NAME=myapp_feature/auth"));
